# Code from:
# https://github.com/AlexiaJM/Deep-learning-with-cats/blob/master/Setting%20up%20the%20data/preprocess_cat_dataset.py
...
def describePositive():
	...
	for imagePath in glob.glob('cats/*.jpg'):
		...
		if min(h,w) >= 64:
			Path1 = imagePath.replace("cats","cats_bigger_than_64x64")
			cv2.imwrite(Path1, crop)
		if min(h,w) >= 128:
			Path2 = imagePath.replace("cats","cats_bigger_than_128x128")
